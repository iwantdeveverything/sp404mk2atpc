    ...
    let samples: Result<Vec<f32>, _> = match spec.sample_format {
        ...
        hound::SampleFormat::Int => {
            if spec.bits_per_sample == 16 {
                reader.samples::<i16>().map(|s| s.map(|v| v as f32 / i16::MAX as f32)).collect()
            } else if spec.bits_per_sample == 24 {
                reader.samples::<i32>().map(|s| s.map(|v| v as f32 / 8388608.0)).collect()
            } else if spec.bits_per_sample == 32 {
                reader.samples::<i32>().map(|s| s.map(|v| v as f32 / i32::MAX as f32)).collect()
            } else {
